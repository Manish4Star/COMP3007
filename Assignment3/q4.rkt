...
;100917191
;I added code to line 47 and from 150 - 160 but it is not working
...
(define (eval exp env)
  (cond ((self-evaluating? exp) exp)
        ...
        ((let? exp) (begin (display "we are checking if let\n")
                           (eval-let exp env))) ;;MY SHIT HERE---------------------------------
        (else
...
(define (let-exps exp) (map cadr (cadr expr)))

(define (let-body exp) (cddr exp))

(define (make-let exp)
  (cons (make-lambda (let-parameters) (let-body)) (let-exps)))

(define (eval-let exp env)
  (eval (make-let exp) env))
